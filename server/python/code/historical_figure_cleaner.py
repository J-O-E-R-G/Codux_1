...
try:
    df_existing = pd.read_csv(os.path.join('./server/python/csvs', 'top100.csv'))
except pd.errors.EmptyDataError:
    df_existing = pd.DataFrame()

df_new = pd.read_csv(os.path.join('./server/python/csvs', 'top100_2.csv'))
...
df_existing.to_csv(os.path.join('./server/python/csvs', 'top100.csv'), index=False)
